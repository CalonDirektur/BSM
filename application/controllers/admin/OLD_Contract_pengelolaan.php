...
class Contract_pengelolaan extends CI_Controller
{
	...
	public function add()
	{
		...
		$this->form_validation->set_rules('masa_awal_kontrak', 'Masa_awal_kontrak Name', 'required');
		$this->form_validation->set_rules('masa_akhir_kontrak', 'Masa_akhir_kontrak Name', 'required');
		$this->form_validation->set_rules('durasi_kontrak_bulan', 'Durasi_kontrak_bulan Name', 'required|integer');
		$this->form_validation->set_rules('luasan_m2', 'Luasan_m2 Name', 'numeric');
		...
		$this->form_validation->set_rules('hrg_sc_per_periodkontrakrp', 'hrg_sc_per_periodkontrakrp Name', 'trim');
		...
		$this->form_validation->set_rules('profit_center', 'Profit_center Name', 'required');
		$this->form_validation->set_rules('status_kontrak', 'Status_kontrak Name', 'required');
		$this->form_validation->set_rules('tax_remark', 'Tax_remark Name', 'required');
$this->form_validation->set_rules("id_file", "Id_file", "trim|xss_clean");

         $this->contract_pengelolaan->uploadData($photo_data, "id_file", "photo_path","","gif|jpg|png|jpeg|pdf");
        ...
        {
			$data["jenis_pengelolaan"]=$this->contract_pengelolaan->getListTable("jenis_pengelolaan");$data["satuan_period_kontrak"]=$this->contract_pengelolaan->getListTable("satuan_period_kontrak");$data["captive_or_non"]=$this->contract_pengelolaan->getListTable("captive_or_non");$data["unit_fm_bm"]=$this->contract_pengelolaan->getListTable("unit_fm_bm");$data["status_kontrak"]=$this->contract_pengelolaan->getListTable("status_kontrak");$data["users"]=$this->contract_pengelolaan->getListTable("users");
			$data['pengguna'] = $this->ion_auth->user()->row();
        ...
        else
        {
			...
			$saveData['hrg_sc_per_periodkontrakrp'] = set_value('hrg_sc_per_periodkontrakrp');
...
if(isset($photo_data["id_file"]) && !empty($photo_data["id_file"]))

		{
	      ...
	      $saveData["id_file"] = $photo_data["id_file"];
				...
				$saveData['updater_by'] = $this->ion_auth->user()->row()->id;
			...
			$saveData['tanggal'] = $now->format('Y-m-d H:i:s');

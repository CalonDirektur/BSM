...
{
    public $v_fields=array('nama', 'nik', 'status_karyawan.status_kary', 'unit', 'sub_unit', 'jabatan', 'users.username');
	...
	function getList($table, $pagination=array()) {
        ...
        $this->db->select("$table.*  , status_karyawan.status_kary as status  , users.username as entry_by ");
        $this->db->from($table);
         $this->db->join("status_karyawan", "karyawan.status = status_karyawan.id", "left");  $this->db->join("users", "karyawan.entry_by = users.username", "left");     

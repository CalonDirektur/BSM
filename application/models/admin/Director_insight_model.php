...
class Director_insight_model extends CI_Model
{
    public $v_fields = ['pesan', 'director_list.jabatan', 'date'];
    ...
    function getList($table, $pagination = [])
    {
        ...
        $this->db->select("$table.*  , director_list.jabatan as director ");

        $this->db->from($table);

        $this->db->join(
            'director_list',
            'director_insight.director = director_list.id',
            'left'
        );
    ...
    function getListTableside($table)
    {
        $this->db->select('*');
